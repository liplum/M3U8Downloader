...
{
    public class M3U8DownloadTaskToolService : IM3U8DownloadTaskToolService
    {
        public M3U8DownloadTaskToolService(IContainerProvider containerProvider)
        {
        ...
        public bool CanStart(M3U8DownloadTask task)
        {
            ...
            return state == TaskState.NOT_STARTED || state == TaskState.STOPPED;
        }
        ...
        public bool CanStop(M3U8DownloadTask task)
        {
            var state = task.State;
            return
                state == TaskState.NOT_STARTED || state == TaskState.STARTED;
        }
        ...
        public bool CanEdit(M3U8DownloadTask task)
        {
            var state = task.State;
            return
                state == TaskState.NOT_STARTED || state == TaskState.STOPPED || state == TaskState.ERROR || state == TaskState.EDITING;
        }
        ...
        public bool CanSave(M3U8DownloadTask task)
        {
            var state = task.State;
            return
                state == TaskState.NOT_STARTED || state == TaskState.ERROR;
        }
    }
}